...
//Notes::: 1. Change to console.local
//         2. For localStorage vs IndexedDB
//         3.
function save_tabs(tabs) {
    ...
    localStorage.setItem("saved_tabs", JSON.stringify(saved_tabs));
...
function fetch_tabs() {
    ...
    saved_tabs = JSON.parse(localStorage.getItem("saved_tabs"));
}
...
function restore_tabs() {
    ...
    var saved_tabs =  new Array();
    saved_tabs = fetch_tabs();
    for (var i = 0; i < saved_tabs.length; i++) {
        chrome.windows.create({focused: true, url: saved_tabs[i]});
    }
